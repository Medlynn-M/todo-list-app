...
def login_ui():
    ...
    password = st.text_input('🔐 Enter your Secret Code', key='login_password', type='password')

    # Forgot code button below password aligned right
    cols = st.columns([3, 1])
    with cols[1]:
        st.markdown("""
        <style>
    ...
    # Style all buttons smaller uniformly
    st.markdown("""
    <style>
    .stButton > button {
    ...
    if st.button('Launch Mission', key='launch_mission'):
        if not username or not password:
            st.error('Enter both Call Sign and Secret Code.')
            return
...
def signup_ui():
    st.header("🛠 Create Your Commander Profile")
    ...
    username = st.text_input('Choose your Call Sign:', key='signup_username', help='Case-sensitive.')
    password = st.text_input('Choose your Secret Code:', key='signup_password', type='password')
    confirm_password = st.text_input('Confirm Secret Code:', key='signup_confirm_password', type='password')
    question = st.selectbox('Select Security Question:', options=SECURITY_QUESTIONS)
    ...
    st.markdown("<span style='color:orange;'>⚠️ Save your security question and answer. This is the only way to recover access if you forget your code.</span>", unsafe_allow_html=True)

    if st.button('Enlist as Commander'):
        ...
        if not is_strong_password(password):
            st.error('Secret Code must be at least 8 characters, including uppercase, lowercase, digit, and symbol.')
            return
...
def forgot_password_ui():
    ...
    if st.session_state['security_verified']:
        ...
        st.caption('Must be at least 8 characters with uppercase, lowercase, number, and symbol.')
        ...
        if st.button('Reset Secret Code'):
            ...
            if reset_user_password(st.session_state['reset_username'], new_password):
                st.success('Secret Code reset successful! Please login.')
                st.session_state['security_verified'] = False
                st.session_state['reset_username'] = ''
                st.session_state['user_record'] = None
...
if not st.session_state['logged_in']:
    if st.session_state['forgot_mode']:
        ...
        if (not st.session_state['show_register_form'] and 
            ...
            if st.button('Enlist as New Commander'):
                st.session_state['show_register_form'] = True
                st.rerun()
...
if not tasks:
    st.info('No missions set. Add your next commands!')

for task in tasks:
    cols = st.columns([9, 1])
    with cols[0]:
        status = "✅" if task["completed"] else "⬜"
        checkbox = st.checkbox(task["task"], value=task["completed"], key=f'task_{task["id"]}')
