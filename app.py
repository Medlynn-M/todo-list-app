...
def add_task(text, date_str, time_str, user):
    table.create(
        {"User": user, "Task": text, "Date": date_str, "TimeSlot": time_str, "Completed": False}
    )

def delete_task(record_id):
    table.delete(record_id)

# Login UI with switched buttons order
def login_ui():
...
time_input = st.text_input("Enter mission time (e.g., 3:15 pm or 15:15)")
